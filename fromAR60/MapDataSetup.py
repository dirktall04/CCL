...
Updated 2014-05-01
@author: kyleg
Modified 2014-10-12 by dtalley
'''
#import data paths from config file.  If testing in ArcMap, run config first to set up environments
try:
    from config_local import (ws, connection0, connection1, citylimits, stateroutelyr, cntyroutelyr, calibrationPointLocation,
        laneclass, maintenance, resolve, LineFeatureClass, NewRouteKey, NewBeg, NewEnd, NewRoute, 
        schema)
except:
    pass
try:
    from KDOT_Imports.dt_functions import Create_CCL_Calibration_Points, ReportResolutionOrdering  # @UnresolvedImport
except:
    print "Failed to import from KDOT_Imports.dt_functions."
    pass

from arcpy import (env, Intersect_analysis, SelectLayerByAttribute_management, DissolveRouteEvents_lr, DeleteRows_management,
                    CreateRoutes_lr, ChangePrivileges_management, CalibrateRoutes_lr, DeleteIdentical_management, Exists,
                    AddJoin_management, Delete_management, FeatureClassToFeatureClass_conversion, MakeFeatureLayer_management,
                    MakeTableView_management, Dissolve_management, AddField_management, CalculateField_management,
                    LocateFeaturesAlongRoutes_lr, MakeRouteEventLayer_lr, OverlayRouteEvents_lr)
import datetime
env.workspace = connection0
...
def setupEnv():
    ...
    OverlayRouteEvents_lr(connection1+"CCL_Resolution","LRS_KEY LINE BEG_CNTY_LOGMILE END_CNTY_LOGMILE",laneclass,"LRS_KEY LINE MIN_BCMP MAX_ECMP","INTERSECT",connection1+"CCL_LANE_CLASS_OVERLAY","LRS_KEY LINE BEG_CNTY_LOGMILE END_CNTY_LOGMILE","NO_ZERO","FIELDS","INDEX")
...
def calibrationCCL_Edit():
    ...
    Intersect_analysis(intersects,calibrationPointLocation,"ALL","#","POINT")
    print calibrationPointLocation
    MakeFeatureLayer_management(calibrationPointLocation, "Calibrators")
    ...
    AddField_management(calibrationPointLocation,"CCL_MEASURE", "DOUBLE", 12, 3)
    ...
    CalibrateRoutes_lr(connection1+"CCL_LRS_ROUTEbase", "CCL_LRS", calibrationPointLocation, "point_LRS", "point_Loc", connection1+"CCL_LRS_ROUTE", 
        "MEASURES", "10 Feet", "BETWEEN", "NO_BEFORE", "NO_AFTER", "NO_IGNORE", "KEEP", "INDEX")
