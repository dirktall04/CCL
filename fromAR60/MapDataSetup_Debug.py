...
Updated 5/1/2014
@author: kyleg
Modified 9/10/2014 by Dtalley
'''
#import data paths from config file.  If testing in ArcMap, run config first to set up environments
try:
    from config import (ws, connection0, connection1, citylimits, stateroutelyr, cntyroutelyr, 
        laneclass, maintenance, resolve, LineFeatureClass, NewRouteKey, NewBeg, NewEnd, NewRoute, 
        schema)
    from dt_functions import Create_CCL_Calibration_Points, ReportResolutionOrdering
except:
    pass

from arcpy import (env, Intersect_analysis, SelectLayerByAttribute_management, DissolveRouteEvents_lr, 
    DeleteRows_management, CreateRoutes_lr, ChangePrivileges_management, CalibrateRoutes_lr, 
    DeleteIdentical_management, Exists, AddJoin_management, Delete_management, 
    FeatureClassToFeatureClass_conversion, MakeFeatureLayer_management, MakeTableView_management, 
    Dissolve_management, AddField_management, CalculateField_management, LocateFeaturesAlongRoutes_lr, 
...
processStartDateTime = datetime.datetime.now()

def setupEnv():
    print "run at "+ str(processStartDateTime)
    ...
    MakeFeatureLayer_management(citylimits, "CityLimits_Made", "TYPE IN ( 'CS', 'ON')")
    LocateFeaturesAlongRoutes_lr("CityLimits_Made","cmlrs","LRS_KEY","0 Feet",connection1+"GIS_CITY","LRS_KEY LINE Beg_CMP End_CMP","FIRST","DISTANCE","NO_ZERO","FIELDS","M_DIRECTON")
...
def calibrationCCL():
    ...
    ##CCLEnd = "!"+schema+"CITY_CONNECTING_LINK_STATE.MAX_END_STATE_LOGMILE! - !"+schema+"CITY_CONNECTING_LINK_STATE_D.MAX_END_STATE_LOGMILE!"
    ...
    #AddField_management("CCL_sections","CCL_BEGIN","DOUBLE","#","#","#","#","NULLABLE","NON_REQUIRED","#")
    AddField_management("CCL_sections","CCL_END","DOUBLE","#","#","#","#","NULLABLE","NON_REQUIRED","#")
    ...
    CalibrateRoutes_lr(connection1+"CCL_LRS_ROUTEbase","CCL_LRS",connection1+"CALIBRATION_POINTS_CCL","CCL_LRS","CCL_MEASURE",connection1+"CCL_LRS_ROUTE","DISTANCE","1 Feet","BETWEEN","NO_BEFORE","NO_AFTER","IGNORE","KEEP","INDEX")
...
# Need to clean this up a bit. Some calculations may not be needed when using the new calibration points.
def calibrationCCL_Edit():
    ...
    #CalculateField_management("Calibrators","CCL_MEASURE","!POINT_M! - !MIN_BEG_STATE_LOGMILE!","PYTHON","#")
    CreateRoutes_lr(LineFeatureClass,NewRouteKey,connection1+NewRoute+"base","TWO_FIELDS",NewBeg, NewEnd,"UPPER_LEFT","1","0","IGNORE","INDEX")
    CalibrateRoutes_lr(connection1+"CCL_LRS_ROUTEbase","CCL_LRS",r"D:\workspaces\pythontests.gdb\CCL_Point_Calibrators","point_LRS","point_Loc",connection1+"CCL_LRS_ROUTE","MEASURES","10 Feet","BETWEEN","NO_BEFORE","NO_AFTER","NO_IGNORE","KEEP","INDEX")
    AddField_management(connection1+NewRoute, "NETWORKDATE", "DATE")
    CalculateField_management(connection1+NewRoute,"NETWORKDATE","datetime.datetime.now( )","PYTHON_9.3","#")
    MakeFeatureLayer_management(connection1+"CCL_LRS_ROUTE", NewRoute)
...
def Maintenance():
    ...
    MakeRouteEventLayer_lr("cmlrs","LRS_KEY", "Maint_tview","LRSKEY LINE BEGMILEPOST END_MP","Maintenance_Events_CNTY","#","ERROR_FIELD","ANGLE_FIELD","NORMAL","ANGLE","LEFT","POINT")
...
#    CityConnectingLink()
    ...
    #ROPrivs()
    processEndDateTime = datetime.datetime.now()
    print "ended at "+ str(processEndDateTime)
    #ScriptStatusLogging()
